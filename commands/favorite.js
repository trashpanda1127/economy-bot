    ...
    Data.findOne({
        ...
        for (let i = 0; i < data.characters.length; i++) {
            if (data.characters[i].id === idToAdd) {
                if (data.favorite === 0) {
                    data.favorite = idToAdd;
                    message.reply(`${data.characters[i].name} is now your favorite character!`)
                    data.save().catch(err => console.log(err));
                    return;
                } else if (data.favorite !== idToAdd) {
                    data.favorite = idToAdd;
                    message.reply(`${data.characters[i].name} is now your favorite character!`);
                    data.save().catch(err => console.log(err));
                    return;
                } else {
                    message.reply(`${data.characters[i].name} is already your favorite character`);
                    return;
                }
            }
        }
